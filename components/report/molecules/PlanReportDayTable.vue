...
const headers: Header[] = [
  { text: t('reports.model.date'), value: 'key', sortable: false },
  { text: t('reports.model.overDuePlans'), value: 'total', sortable: false },
]

const dayjs = useDayjs()
const startOfDay = ref(dayjs().format('YYYY-MM-DD'))
const endOfDay = ref(dayjs().format('YYYY-MM-DD'))

const queryBuilder = computed(() => ({
  filters: {
    type: 'day',
    date_from: dayjs(startOfDay.value).format('YYYY-MM-DD'),
    date_to: dayjs(endOfDay.value).format('YYYY-MM-DD'),
  },
}))
...
<template>
  <div>
    <div class="d-flex white-space tw-gap-2 tw-py-[4px] tw-mb-1">
      <date-picker v-model="startOfDay" class="tw-h-[32px]" />
      <date-picker v-model="endOfDay" class="tw-h-[32px]" />
    ...
    >
      ...
      <template #item-key="{ key }">
        <span>{{ dayjs(key).format('DD/MM/YYYY') }}</span>
      </template>
