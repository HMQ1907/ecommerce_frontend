...
import useGetDashboardFluctuateSalary from '@/composables/use-get-dashboard-fluctuate-salary'
...
const { items, refetch, isLoading, isFetching } = useGetDashboardFluctuateSalary(query.value)
    ...
    chart.value.updateSeries([
      {
        ...
        data: Object.entries(val.current.salary).map((item: any) => round(item[1], 2)),
      },
    ])
...
<template>
  <v-card class="tw-relative" variant="outlined">
    <div class="d-flex tw-p-1">
      <v-card-title>{{ $t('employees.model.totalEmployeeSalaryFluctuate') }}</v-card-title>
      <v-spacer />
